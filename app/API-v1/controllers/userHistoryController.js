export class UserHistoryController {
  ...
  async createUserHistory(req, res){
    ...
    let userHistory = await this._userHistoryService.create({...body, status: "Activo"});
  ...
  async updateUserHistory(req, res){
    ...
    res.status(201).send({message: "Updated userHistory"});
  ...
  async deleteUserHistory(req, res){
    ...
    res.status(201).send({message: "Deleted userHistory"});
